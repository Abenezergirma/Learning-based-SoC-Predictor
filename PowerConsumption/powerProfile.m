...
speed = 5;
timeInterval = 5;
wayPoints = aircraft.generateWaypoints(initialXY, finalXY, altitude, speed, timeInterval);
...
for t = 0:update_interval:total_time
    ...
    elseif isempty(power_cruise)
        ...
        power_projection = [power_climb(2:end); power_cruise]; % Skip duplicate first climb value
    end

    % **Ensure time and power arrays match in length**
    min_length = min(length(time_projection), length(power_projection));

    if min_length > 0
        time_projection = time_projection(1:min_length);
        power_projection = power_projection(1:min_length);
    else
        fprintf('Warning: Empty projection at iteration %d\n', t/update_interval);
    end

    % **Remove duplicate times & sort**
    [time_projection, unique_indices] = unique(time_projection, 'stable');
    power_projection = power_projection(unique_indices);

    % **Interpolate to match the 0.1 sec time step**
    if ~isempty(time_projection) && length(time_projection) > 1
        time_interp = time_projection(1):0.1:time_projection(end);
        power_interp = interp1(time_projection, power_projection, time_interp, 'linear', 'extrap');
        power_interp(2:end) = power_interp(end);
    else
        time_interp = time_projection;
        power_interp = power_projection;
    end

    % **Store interpolated projected profile dynamically**
...
function plotRealTimePowerProfile(projected_profiles_list)
    ...
    if ~isempty(projected_profiles_list{end}.time)
