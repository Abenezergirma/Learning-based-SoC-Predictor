...
class TarotBattery:
    ...
    def generate_dataset(self, current_input, num_training=70, num_test=30, scaling_range=(0.6, 1.2)):
        ...
        for i in tqdm(range(num_training), desc="Training Trajectories"):
            ...
            print(f"Iteration {i+1}: Scaling factor = {scale_factor:.4f}")  # Print scaling factor
...
# Load the original current trajectory
current_input = np.loadtxt('Iapp_multirotor')  # Replace with the actual path if needed

# Undersample the original current trajectory to match a time step of 0.1 seconds
...
# Save or use the training and test data as needed
